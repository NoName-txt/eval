...
exports.run = (client, message, args, lang) =>  {
    ...
    if(args[0] == "add") {
        ...
        if(db.has("access")) if(!db.get("access").includes(usr)) db.push("access", usr);
            else db.push("access", usr);
        message.delete();
        const emb = new Discord.MessageEmbed().setDescription(`**Access Added** \n\n<@${usr}>`).setFooter(message.author.tag,message.author.displayAvatarURL({dynamic:true}));
        ...
        db.pull("access", usr);
        message.delete();
        const emb = new Discord.MessageEmbed().setDescription(`**Access Removed** \n\n<@${usr}>`).setFooter(message.author.tag,message.author.displayAvatarURL({dynamic:true}));
        return message.channel.send({ embeds: [emb] }).then(m => setTimeout(function(){m.delete()},5000));
    }
        ...
        db.set("channel", chn);
        message.delete();
        const emb = new Discord.MessageEmbed().setDescription(`**Channel Changed** \n\n<#${chn}>`).setFooter(message.author.tag,message.author.displayAvatarURL({dynamic:true}));
        return message.channel.send({ embeds: [emb] }).then(m => setTimeout(function(){m.delete()},5000));
    }else{
